...
class Fdn::OrganizationsController < ApplicationController
  skip_before_filter :require_user, :only => ['jq_receive']
  protect_from_forgery except: [:rjs_all_of, :rjs_ent_of, :rjs_descendants_of, :rjs_tree_all_of]

  def layer_select_org

  end

  def layer_select_org_js
  ...
  def import_data
    ...
    if !params[:file].blank?
      ...
      #spreadsheet = Spreadsheet.open params[:file]
      path = Rails.root.join('public', '企业信息.xls')
      spreadsheet = Spreadsheet.open path
      sheet = spreadsheet.worksheet 0
      sheet.each do |row|
        if row[1]
          ent = Fdn::Enterprise.where("ent_code = ?", row[1].to_s.gsub(/\.0/, '')).first
          if ent.nil?
            ent = Fdn::Enterprise.new(ent_code: row[1].to_s.gsub(/\.0/, ''))
          end
          if ent.organization.nil?
            org = ent.build_organization
          else
            org = ent.organization
          end
          ent.ent_level_code = row[2]
          org.code = row[1].to_s.gsub(/\.0/, '')
          org.name = row[0]
          org.short_name = row[0]
          ent.save(validate: false)
          #@parent_org = Fdn::Organization.where(code:row[3].to_s.gsub(/\.0/,'')).first
          #if @parent_org
          #  if ent.parent
          #    org.with_hierarchy.change_parent(@parent_org)
          #  else
          #    @parent_org.with_hierarchy.add_child(org.id, nil, nil)
          #  end
          #end
          if !row[3].blank?
            @kb_orgs << [org, row[3].to_s.gsub(/\.0/, '')]
          end
          if !row[4].blank?
            @nb_orgs << [org, row[4].to_s.gsub(/\.0/, '')]
          end
          if !row[5].blank?
            @ys_orgs << [org, row[5].to_s.gsub(/\.0/, '')]
          end
          if !row[6].blank?
            @js_orgs << [org, row[6].to_s.gsub(/\.0/, '')]
          end
        end
      end
      sheet2 = spreadsheet.worksheet 1
      sheet2.each do |row|
        org = Fdn::Organization.where(code: row[2].to_s.gsub(/\.0/, '')).first
        if org
          if org.resource_type == 'Fdn::Dept'
            dept = org.resource
            dept.type_code = row[1]
            org.name = row[0]
            org.short_name = row[0]
            dept.save(validate: false)
          end
        else
          dept = Fdn::Dept.new(type_code: row[1])
          org = dept.build_organization
          org.name = row[0]
          org.code = row[2].to_s.gsub(/\.0/, '')
          org.short_name = row[0]
          dept.save(validate: false)
        end
        if !row[3].blank?
      ...
      root = Fdn::Organization.first
      #root.add_children(2,@kb_orgs)
      #root.add_children(3,@ys_orgs)
      #root.add_children(4,@nb_orgs)
      root.add_children(5, @js_orgs)
    end
    respond_to do |format|
      format.html { render :js => view_context.close_window_show_tips('parent.MAIN_LAYER_WINDOW', '企业导入成功！') }
    end
  end
  ...
  def jq_receive
    render :text => "0,"+request["UserName"]
  end

  def visit_jq
    @name = !params[:name].blank? ? params[:name] : ''
    if @name.blank?
      render :partial => 'fdn/organizations/name_wrong'
    else
      remote_url = "http://192.168.43.130:9898/possession/login,hfgzw,0,#{@name}/"
      uri = URI(remote_url)
      ...
      case (res)
        when Net::HTTPSuccess then
          ...
          render :partial => 'fdn/organizations/jq_wrong'
      end
    end
  end
  ...
  def tree_index
    ...
    if node && node != '9999' && node != '99999'
      @level = params[:n_level].to_i+1
  ...
  def edit
    ...
    @type = (@fdn_organization.resource_type == 'Fdn::Dept' ? @obj.type_code : 'ENT')
  ...
  def rjs_ent_of
    ...
    render :js => rjs_by_collection(Fdn::Organization.search_ent, params[:columns], 'ents')
  end
  ...
  def rjs_descendants_of
    current_org.with_hierarchy
    if current_org.is_ent?
      render :js => rjs_by_collection([current_org]+current_org.with_hierarchy.all_descendants, params[:columns], 'descendants')
    else
      render :js => rjs_by_collection(current_org.with_hierarchy.all_descendants, params[:columns], 'descendants')
    end
  end

  def rjs_tree_all_of
    @hie_id = params[:hie_id]
    @hie_name = params[:hie_name]
    @org = Fdn::Organization.first
    render :js => rjs_by_collection([@org]+@org.with_hierarchy(@hie_id).all_descendants, params[:columns], @hie_name)
  end
