...
module Bootstrap3::BootstrapHelper
  ...
  def colorpicker_include_tag
    output = javascript_include_tag('/js/plugins/colorpicker/bootstrap-colorpicker.min.js')
    output << stylesheet_link_tag('/css/plugins/colorpicker/css/bootstrap-colorpicker.min.css')
  end
  ...
  def links(links, seperator=nil)
    ...
    links.each do |l|
      ...
      elsif l[:url_method]== 'get'
        result << bs_a(l)
      else
        result << bs_link_window(l[:url], l[:value], '', '', l[:title]||l[:value])
      end
    end
