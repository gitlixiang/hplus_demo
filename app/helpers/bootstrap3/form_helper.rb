...
module Bootstrap3::FormHelper
  ...
  class LayoutFormBuilder < FormBuilderBase
    ...
    def controls(label, method=nil, &proc)
      ...
      div = label.blank? ? "" : @template.content_tag('label', label, class: "control-label col-sm-#{label_span}")
      #self.label(method, label, class: "control-label span#{label_span}")

      div << @template.content_tag('div', nil, class: "col-sm-#{label.blank? ? 12 : value_span}") do
  ...
  #       no_return: true, false 不显示保存按钮，默认false
      ...
      new_proc = Proc.new { |f|
        ...
        if nav
          form_content << button_div do
            ...
            buttons << bs_save unless nav[:no_save]
            buttons << (nav[:return_url].blank? ? bs_close : bs_return(nav[:return_url]))  unless nav[:no_return]
