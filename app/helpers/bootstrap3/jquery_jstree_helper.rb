...
module Bootstrap3::JqueryJstreeHelper
  ...
  #search_id：查询输入框的id
  def js_tree_org_json_tag(obj_id, remote_url, use_id, search, search_id, other_search={})
    script = Array.new
    ...
    if !remote_url.blank?
      ...
      if other_search.blank?
        url = "        $.jq_remote_get('#{remote_url}',{'#{use_id}': r, format : 'js'});"
      else
        url = "        $.jq_remote_get('#{remote_url}',{'#{use_id}': r"
        other_search.each do |key, value|
          url += ",#{key} : $('##{value}').val()"
        end
        url+=", format : 'js'});"
      end
    ...
    # if other_search.blank?
    #   url =  "'/fdn/roles/org_tree.json?org_id='+node.id;"
    # else
    #   url =  "'/fdn/roles/org_tree.json?id='+node.id"
    #   other_search.each do |key,value|
    #     url += "+'&#{key}='+$('##{value}').val()"
    #   end
    #   url+=';'
  ...
  #权限菜单树
  def js_right_tree_tag(obj_id, data)
    script = Array.new
