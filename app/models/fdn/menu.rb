module Fdn
  class Menu < ActiveRecord::Base
    ...
    #选择权限时 不在企业端显示的菜单
    ENT_NENUS = ['corporate_bond_analysis', 'talent_analysis', 'talent_base', 'talent_reviewed', 'training_courses', 'hr_training_courses',
                 'training_infos', 'fdn_organizations_tree', 'fdn_loggers', 'fdn_menus', 'fdn_announcements', 'param', 'lookups', 'fdn_legends'
    ]
    ...
    scope :code_not_in, lambda { |codes| where('fdn_menus.code not in (?)', codes) }
    ...
    def self.build_menu(current_user)
      ...
      items = []
      i = 1
      top_menus = Fdn::Menu.where(depth: 1).order(:position)
      top_menus.each do |m|
    ...
    def menu_html_str(current_user, top=false, is_last_top=false)
      ...
      if !items.blank?
        item_str = "<ul class='nav nav-#{depth+1}-level collapse'>" + items.join + "</ul>"
        #if top
      ...
      if code == Fdn::Menu::DIVIDER_MENU
        ...
        if top
          ...
          if item_str.blank?
            ...
            return "<li><a class='J_menuItem' href='#{url}'  id='#{code}'><i class='fa fa-#{title_img.blank? ? 'gear' : title_img}'></i> #{name}</a></li>"
          else
            return "<li><a  href='#'  id='#{code}'><i class='fa fa-#{title_img.blank? ? 'gear' : title_img}'></i> <span class='nav-label'>#{name}</span><span class='fa arrow'></span></a>#{item_str}</li>" #+ (is_last_top ? "" : "<li class='spacer'><span>|</span></li>")
          ...
          #判断是否有下级菜单
          if item_str.blank?
            return "<li ><a class='J_menuItem' href='#{url}' id='#{code}'>#{name}</a></li>"
            #return "<li ><a href='#{url}' id='#{code}'>#{name}</a></li>"
          else
            return "<li ><a  href='#' id='#{code}'>#{name}<span class='fa arrow'></span></a>#{item_str}</li>"
