...
module Fdn
  class Organization < ActiveRecord::Base
    ...
    HIERARCHY_TREE_PARAMS = []
    Fdn::OrgHierarchy.all.each do |hie|
      HIERARCHY_TREE_PARAMS << [hie.id, "#{hie.code}_id"]
    end

    SEARCH_ENT_ARR = [["组织机构代码", "code"], ["企业级次", "ent_level_value"], ["组织形式", "org_type"], ["注册资本（万元）", "reg_amt"], ["主要出资人出资比例", "percentage"], ["主要行业", "main_ind_value"], ["企业类别", "ent_type_value"]]
    ATTR_ARR = ["企业名称", "产权登记情形", "注册资本（万元）", "设立注册日期", "是否已办工商", "组织机构代码", "工商登记日期", "主要出资人组织机构代码", "企业类别", "组织形式", "主要行业", "经营状况", "是否国家出资企业主业", "注册地"]
    ...
    has_many :people, :class_name => 'Hr::Person', :foreign_key => 'org_id' #, :through => :hr_assignments
    ...
    scope :search_ent, -> { where('resource_type = \'Fdn::Enterprise\'') }
    ...
    def create_default_user
      ...
      if self.id == 1
        ...
        self.users.create(FactoryGirl.attributes_for(:jw_user)).create_user_information(FactoryGirl.attributes_for(:jw_user_info))
        ...
        self.users.create(FactoryGirl.attributes_for(:leader_user, :init_pass)).create_user_information(FactoryGirl.attributes_for(:leader_info))
      else
        self.users.create(FactoryGirl.attributes_for(:fdn_user, :init_pass)).create_user_information(FactoryGirl.attributes_for(:ent_user_info))
        self.users.create(FactoryGirl.attributes_for(:adv_user)).create_user_information(FactoryGirl.attributes_for(:ent_adv_user_info))
      end
    end
    ...
    def create_default_role
      ...
      #in_menu_codes(Fdn::Menu::KB_ADMIN_MENUS)
      ...
      #end
      if self.is_ent?
        all_rights = Fdn::Right.all.not_in_menu_codes(Fdn::Menu::ENT_NENUS)
      else
        all_rights = Fdn::Right.all
      end
      if self.id == 1
        jw_user = Fdn::User.by_org_id(self.id).like_user_name('jiwei').first
        leader = Fdn::User.by_org_id(self.id).like_user_name('leader').first
        Fdn::Role.init_role_with_org_and_user(self, jw_user, Fdn::Role::DEFAULT_JW_ROLES, all_rights)
        Fdn::Role.init_role_with_org_and_user(self, leader, Fdn::Role::DEFAULT_LEADER_ROLES, all_rights)
      end
    ...
    def add_children(hierarchy_id, all_orgs)
      @parent_org = self
      orgs = []
      all_orgs.map { |o| orgs << o if o[1] == @parent_org.code }
      orgs.compact!
      if orgs.size != 0
        orgs.each do |child|
          @child_org = child[0]
          if @parent_org.code == child[1]
            if @child_org.with_hierarchy(hierarchy_id).root.blank?
              @parent_org.with_hierarchy(hierarchy_id).add_child(@child_org.id, nil, nil)
            else
              @child_org.with_hierarchy(hierarchy_id).change_parent(@parent_org.id, nil)
            end
          end
          #TODO
          @child_org.add_children(hierarchy_id, all_orgs)
        end
      end
    end
