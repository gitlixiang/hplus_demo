...
module Fdn::OrganizationDomain::Tree
  ...
  def self.included(base)
    ...
    base.class_eval do
      ...
      scope :se_descendants, lambda { |hierarchy_id, time, child_id| join_hierarchy_from_parent.where_hierarchy(hierarchy_id, time).where(
          'fdn_org_hie_elements.parent_id=? and fdn_org_hie_elements.distance >= 1', child_id
      ).order('fdn_org_hie_elements.distance').select_hierarchy(time) }
