      ...
      if  self.resource_type == 'Fdn::Enterprise'
        Fdn::OrgHierarchy.all.each do |hie|
          if self.with_hierarchy(hie.id, time).parent
            #value += "<span class='badge' style='background-color:#{hie.icon}' title='#{hie.short_name}'>&nbsp;</span>&nbsp;"
            value += "<button type='button' title='#{hie.short_name}' class='btn btn-sm btn-rounded' style='background-color:#{hie.icon}'></button>&nbsp;"
          end
        end
      end
