...
ActiveRecord::Base.connection_pool.with_connection do |c|
  ...
  birth_date = Proc.new { Time.now.change(year: rand(1970..1995), month: rand(1..12), day: rand(1..28)) }
  ...
  Fdn::Enterprise.all.each do |ent|
    org = ent.organization
    proc = Proc.new { org.people.create(sex: (s = sex.call), full_name: name.call(s), birth_date: birth_date.call, nationality_code: 01, job_id: job_id.call, myself_status: myself_status.call, entry_ways_code: entry_ways_code.call, entry_ways_date: entry_ways_date.call) }
    case ent.ent_level_code
      when '1'
        1000.times { proc.call }
      when '2'
        500.times { proc.call }
      when '3'
        100.times { proc.call }
